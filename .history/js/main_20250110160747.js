...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function onThreeJSResize() {
        if (camera && renderer) {
            ...
            if (logo) {
                const aspect = logo.geometry.parameters.width / logo.geometry.parameters.height;
                let baseSize = 5; // Default size for desktop

                // Adjust size for mobile devices
                if (window.innerWidth < 500) {
                    baseSize = 2; // Extra small size for very small screens
                ...
                logo.geometry = new THREE.PlaneGeometry(baseSize * aspect, baseSize);
            }
        }
    }
